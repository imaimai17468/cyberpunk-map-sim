---
doc_id: adr-0007
title: TanStack Start Migration
kind: reference
...
>
> **Amendment (2026-08-01, `/remove-db` applied):** the bullet above describing
> bindings reaching handlers through `getCloudflareEnv()` in
> `src/server/cloudflare.ts` no longer applies — this fork has no D1 or R2
> binding and that file is deleted. The Worker integration, the
> `wrangler.toml#main` source entry, `createFileRoute` + `server.handlers`, and
> `vite build && wrangler deploy` all still describe the code. Only the binding
> sub-mechanism lapsed; the migration decision itself stands.
