...
> **Amendment (2026-08-01, `/remove-db` applied):** the standing exception
> permitting a real `CLOUDFLARE_API_TOKEN` in the working tree existed only
> for drizzle-kit's remote schema commands. drizzle-kit is removed, so the
> exception is closed and every secret now goes through `wrangler secret put`
> with no exceptions. This ADR's own acceptance test — that the remote path
> works with the token slot empty — is satisfied trivially: there is no
> remote schema path.

## Context

ADR-0004 made `permissions.deny`
the security boundary, and ADR-0013
