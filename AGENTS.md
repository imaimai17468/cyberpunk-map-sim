...
- **Work in commit-sized units; land each before starting the next.** Splitting is a property of how the work was done, not a shape imposed on it afterwards. A large batch carved up at the end is the expensive path even when it succeeds, and the carving is what goes wrong.
- **Split by staging only. Never edit files back to an intermediate state to make one.** `git add -p` accepts its answers from a pipe here, so splitting by staging genuinely works (checked 2026-08-02: `printf 'y\nn\n' | git add -p <file>` on a two-hunk file put the first hunk in the index and left the second only in the working tree, and the file on disk was untouched. The ban on interactive flags covers `git add -i`, not `-p`). Rewriting a file to reconstruct "what it looked like at commit 1" instead is what breaks: the review stamp is bound to the working tree, and ADR-0013 and ADR-0019 both rest on *committing does not edit files* — that premise is the reason one review covers a multi-commit split at all. Break it and the review is void, so the split costs a whole extra review cycle. `remove-db`'s note that intermediate commits are not individually buildable records a consequence of deletion order; it is not licence to manufacture intermediate states.
- If a split cannot be made by staging alone, **make it one commit** and say why in the body. One honest commit beats a fabricated history.
