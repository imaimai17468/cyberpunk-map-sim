...
| 8 | P2 emergent building count + retry loop (J1/J3) | **Resolved.** Closed-form count control: before lot subdivision, `expected = Σ blockArea/targetLotArea(district)`; a single deterministic `lotAreaScale = clamp(expected/5500, 0.6, 1.8)` is applied once. No retry, no second pipeline run. Invariant test asserts the band on fixture seeds. |
